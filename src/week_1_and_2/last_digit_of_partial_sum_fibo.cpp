...
int get_fibonacci_partial_sum_naive(unsigned long long from, unsigned long long to) {
    int sum = 0;
...
void StressTest(int numIter, unsigned long long range_n)
{
    ...
    for(i=0;i<numIter;++i)
    {
        ...
        if(get_fibonacci_partial_sum_naive(m,n) != get_fibonacci_partial_sum_fast(m,n))
        {
...
int main() 
{
    ...
    assert(get_fibonacci_partial_sum_fast(0,k_long) == get_fibonacci_sum_fast(k_mod_60_short));
