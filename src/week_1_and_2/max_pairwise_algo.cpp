...
unsigned long long MaxPairwiseProductNaive(const vector<unsigned long long>& numbers) {
...
unsigned long long MaxPairwiseProductFast2(vector<unsigned long long>& numbers) {
  int n = numbers.size();
  std::stack<int> stack;
  stack.push(0);
  if (n==2) { return numbers[0]*numbers[1];}
  ...
  for (int i = 1; i < n; ++i)
  {
    if (numbers[i]>=numbers[stack.top()]) { stack.push(i); }
  }
